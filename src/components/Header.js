  ...
  return (
    <div className="flex justify-between items-center bg-gray-900 text-white px-4 py-2">
      ...
      <span className="text-black flex-grow px-3">
        <form className="flex">
          <select
            className="rounded-l text-xs p-2 bg-gray-200 border border-gray-400"
            id="items"
          >
  ...
  return (
    <div className="flex flex-col">
        ...
        <div className="absolute h-screen w-screen">
          <div className="flex h-full w-full">
            <div ref={side} className="flex flex-col float-left bg-white">
              <span className="bg-gray-700 flex w-96 pl-8 py-3 items-center">
                {" "}
                <img src={profile} alt="" />
                <p className="font-bold text-xl text-white pl-2">
                  Hello, Sign in
                </p>
              </span>
            ...
            <div
              ...
              className="flex w-full bg-black opacity-70"
            ></div>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-start bg-gray-700 text-white p-2">
        ...
        >
          <img className="w-5 mr-2" src={hamburger} alt="" />
          <p>All</p>
        </button>
        {categories.map((a) => (
          <div className="px-3">{a}</div>
        ))}
      </div>
    </div>
  );
